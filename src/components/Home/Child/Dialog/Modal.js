...
class Modal extends React.Component {
  ...
  static getDerivedStateFromProps(props, state) {
    if (props.objectType !== state.title) {
      if (
        props.objectType === config.offlineVehicles ||
        props.objectType === config.onlineVehicles
      ) {
  ...
  componentDidUpdate(prevProps, prevState) {
    if (prevProps.objectType !== this.props.objectType) {
      if (this.props.objectType)
        if (
          this.props.objectType === config.offlineVehicles ||
          this.props.objectType === config.onlineVehicles
        )
          if (
            this.compareDateTimeRange(this.state.startDate, this.state.endDate)
          ) {
  ...
  updateDateRange = (date, key, title) => {
    ...
    if (key === "endDate") {
      if (
        ...
        this.compareDateTimeRange(this.state.startDate, date)
      ) {
    ...
    if (this.compareDateTimeRange(this.state.startDate, this.state.endDate)) {
      let status =
        title === config.onlineVehicles ? config.OnlineLog : config.OfflineLog;
      ...
      <div className="search-part d-flex justify-content-between">
        ...
        <div style={{ display: "flex" }}>
          ...
          <button
            ...
            style={{ marginLeft: "5px", border: "black 1px solid" }}
            onClick={() => this.getCheckinsReport()}
          >
    ...
    return (
      <div className="col-md-12">
        ...
        <div className="row col-md-12" style={{ padding: "3%" }}>
          ...
          <p>
            Provisioning refers to the process of enrolling a device with an
            enterprise and applying the appropriate policies to the device.
            Before attempting to provision a device, ensure that the device is
            running Android 6.0 or above. You need an enrollment token for each
            device that you want to provision (you can use the same token for
            multiple devices), when creating a token specify a policy that will
          ...
          <ol>
            ...
            <li>
              Tap Next, and then accept the installation of Android Device
              Policy.
            </li>
              ...
              <table>
                ...
                >
                  ...
                  >
                    ...
                    <th onClick={(e) => this.toggleSort("plateNumber")}>
                      Plate Number{" "}
                      <span
                        className={
                          this.state.sortPlateNumber
                            ? "glyphicon glyphicon-sort-by-attributes"
                            : "glyphicon glyphicon-sort-by-attributes-alt"
                        }
                      ></span>
                    </th>
                    <th onClick={(e) => this.toggleSort("days")}>
                      Days{" "}
                      <span
                        className={
                          this.state.sortDays
                    ...
                    <th onClick={(e) => this.toggleSort("total")}>
                      Hours{" "}
                      <span
                        className={
                          this.state.sortHours
                            ? "glyphicon glyphicon-sort-by-attributes"
                            : "glyphicon glyphicon-sort-by-attributes-alt"
                        }
                      ></span>
                    </th>
                    <th onClick={(e) => this.toggleSort("institutionName")}>
                      Institution{" "}
                      <span
                        className={
                          this.state.sortInstitution
                            ? "glyphicon glyphicon-sort-by-attributes"
                            : "glyphicon glyphicon-sort-by-attributes-alt"
                        }
                      ></span>
  ...
  returnIdforObjectType = (object, objectType) =>
  ...
  toggleSort = (key) => {
    ...
    if (key === "institutionName") {
      ...
      this.setState({
        ...
        sortDays: false,
        sortPlateNumber: false,
        sortInstitution: sort,
      });
    }
    const order = sort
      ? config.sortOrder.ascending
  ...
  render() {
    ...
    const verifyContextForTracking = (title) => {
      return (
        title === config.offlineVehicles || title === config.onlineVehicles
      );
    };
    ...
    return (
      <div className="modalNew">
        <div
          className={`modal-content${title === "Emm Console" ? " wider" : ""}`}
        >
          <div className="top-part-vehicles-search model-header">
            <span
              className="closeCrudBtn"
            ...
            <div className="header-add-butt">
              <h3>
                ...
                {this.props.objectList &&
                  ...
                  `${validate(this.props.objectList.total)}`}
              </h3>
            </div>
            <hr />
            {verifyContextForTracking(title) && this.returnSearch(title)}
          </div>
          {title === "Enrollment Token" ? (
            this.returnEnrollmentdetails(
              this.props.objectList.value,
              this.props.objectList.policyName
            )
          ) : title === "Emm Console" ? (
            this.returniFrame(this.props.objectList)
          ) : title === config.onlineVehicles ||
            title === config.offlineVehicles ? (
            this.state.list.length == 0 || this.state.loading ? (
              <Launch />
            ) : (
              this.returnVehicles(this.state.list)
            )
          ) : (

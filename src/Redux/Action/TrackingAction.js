...
export const Subscribing = (payload) => ({
  type: trackingConstants.Tracking_OnSubscribeRequest,
});

export const Connected = (payload) => ({
  type: trackingConstants.Tracking_Connected,
});
  ...
  return (dispatch) => {
    ...
    if (hubConnection.state === 0) {
      hubConnection
        ...
        .then(() => {
          ...
          const institutionForPriviledge = isSU(role)
            ? null
            : user.institution.institutionid;
          console.log("institutionForPriviledge ", institutionForPriviledge);
          hubConnection
            .invoke("Subscribe", institutionForPriviledge, null, null)
            .catch(function (err) {
    ...
    hubConnection.on("FeedsReceiver", (result) => {
      ...
      FormatedRes = {
        ...
        status: "active",
        coordinates: {
          lat: parseFloat(res.coordinates.latitude),
          lng: parseFloat(res.coordinates.longitude),
          timestamp: res.coordinates.timestamp,
        },
      };
      //     }
      // }
...
export const Unsubscribe = (payload) => ({
  type: trackingConstants.Tracking_OnUnSubscribeRequest,
});
export const Disconnected = (payload) => ({
  type: trackingConstants.Tracking_Disconnected,
});
...
export function getVehiclesLog(start, end, status, sort, user, role) {
  return (dispatch) => {
    ...
    const authorize =
      role && role.privilege !== config.SU
        ? `institutions/${user.institution.institutionid}/`
        : "";
    dispatch(OfflineDataRequest());

    // axios.get(`https://localhost:5001/v1.0/${authorize}${status}?startAt=${start}&endAt=${end}${sort}`)
    apiHandler
      .get(`${authorize}${status}?startAt=${start}&endAt=${end}${sort}`)
      .then(
        (vehicles) => {
...
function updateVehicleLog(vehicles) {
  return {
    type: trackingConstants.TrackingVehicleLog_Success,
    payload: vehicles,
  };
}
...
export function getDevices(role, user) {
  return (dispatch) => {
    ...
    apiHandler.get(`${domain}?offset=1&limit=10`).then(
      (devices) => {
...
function OfflineUpdateReceived(result) {
  return {
    type: trackingConstants.Tracking_OfflineDataSynced,
    payload: result,
  };
}
export const OfflineDataRequest = () => ({
  type: trackingConstants.Tracking_OfflineDataRequest,
});
export const OfflineDataError = (payload) => ({
  type: trackingConstants.Tracking_OfflineDataError,
});
...
function UpdatedMarkerId(vehicleID) {
  return {
    type: trackingConstants.Tracking_MarkerHighLighted,
    payload: vehicleID,
  };
}
